    ...
    <section className="py-32 bg-white overflow-hidden" id="solutions">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-20 max-w-3xl mx-auto">
          <h2 className="text-4xl md:text-6xl font-black text-blue-950 mb-8">Specialized Focus. <br />Universal Power.</h2>
          ...
          <div className="w-full lg:w-1/3 flex flex-col gap-4">
            <button 
              ...
              className={`p-8 rounded-[2rem] text-left transition-all duration-500 border-2 ${
                tab === 'edu' 
                ? 'bg-blue-900 text-white border-blue-900 shadow-2xl shadow-blue-900/30 -translate-y-2' 
                : 'bg-slate-50 text-slate-500 border-slate-100 hover:bg-slate-100'
              }`}
            >
              <GraduationCap className={`w-10 h-10 mb-4 ${tab === 'edu' ? 'text-orange-400' : 'text-slate-400'}`} />
              <h3 className="text-2xl font-black mb-2">Education Specialty</h3>
              <p className={`text-sm font-medium ${tab === 'edu' ? 'text-blue-100' : 'text-slate-400'}`}>Purpose-built for academies, coaching centers, schools, and universities.</p>
            </button>
            ...
            >
              <Briefcase className={`w-10 h-10 mb-4 ${tab === 'ent' ? 'text-orange-400' : 'text-slate-400'}`} />
              <h3 className="text-2xl font-black mb-2">Business Suite</h3>
              <p className={`text-sm font-medium ${tab === 'ent' ? 'text-blue-100' : 'text-slate-400'}`}>Scalable CRM, HRMS, Accounting and Operations for modern firms.</p>
            </button>
          </div>
          ...
          <div className="w-full lg:w-2/3 bg-slate-50 rounded-[3rem] p-8 md:p-16 relative overflow-hidden min-h-[600px]">
            ...
            <div className={`transition-all duration-700 ${tab === 'edu' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10 absolute inset-0'}`}>
               ...
               <div className="flex flex-wrap gap-4">
                 <button className="px-8 py-4 bg-blue-900 text-white rounded-2xl font-bold shadow-xl shadow-blue-900/30 hover:bg-blue-950 transition-colors flex items-center gap-2">
