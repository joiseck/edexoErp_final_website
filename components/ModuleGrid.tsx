...
export const ModuleGrid: React.FC = () => {
  ...
  return (
    <section className="py-16 bg-slate-900 text-white overflow-hidden relative" id="modules">
      <div className="max-w-7xl mx-auto px-4 relative">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-10 gap-6">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-2 text-orange-400 font-bold text-[10px] uppercase tracking-widest mb-2">
              <div className="w-6 h-[1.5px] bg-orange-500"></div>
              A Complete Ecosystem
            </div>
            <h2 className="text-3xl md:text-4xl font-black mb-3 text-white">Integrated Modules</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {modules.map((m) => (
            <div key={m.id} className="group relative bg-white/[0.03] border border-white/10 p-6 rounded-3xl hover:bg-white/[0.05] hover:border-blue-500/50 transition-all duration-300">
              <div className={`${m.color} w-12 h-12 rounded-xl flex items-center justify-center text-white mb-6 group-hover:scale-105 transition-transform duration-300`}>
                {m.icon}
              </div>
              <h3 className="text-xl font-black mb-2 text-white group-hover:text-blue-400 transition-colors">{m.title}</h3>
              <p className="text-slate-400 mb-6 leading-relaxed text-xs">{m.description}</p>
